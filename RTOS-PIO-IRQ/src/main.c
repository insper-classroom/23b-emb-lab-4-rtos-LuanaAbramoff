...
SemaphoreHandle_t xSemaphoreBut_2;
SemaphoreHandle_t xSemaphoreBut_3;
...
void but2_callback(void) {
	BaseType_t xHigherPriorityTaskWoken = pdFALSE;
	xSemaphoreGiveFromISR(xSemaphoreBut_2, &xHigherPriorityTaskWoken);
}

void but3_callback(void) {
	BaseType_t xHigherPriorityTaskWoken = pdFALSE;
	xSemaphoreGiveFromISR(xSemaphoreBut_3, &xHigherPriorityTaskWoken);
}
...
static void task_but(void *pvParameters) {
	...
	for (;;) {
		...
		if (xSemaphoreTake(xSemaphoreBut_2, 1000)) {
			/* atualiza frequencia */
			delayTicks += 50;
		...
		if (xSemaphoreTake(xSemaphoreBut_3, 1000)) {
			/* atualiza frequencia */
			delayTicks += 10;
		...
		if (xSemaphoreTake(xSemaphoreBut_1, 1000)) {
			/* atualiza frequencia */
			delayTicks += 100;
...
int main(void) {
	...
	xSemaphoreBut_2 = xSemaphoreCreateBinary();
	xSemaphoreBut_3 = xSemaphoreCreateBinary();
	...
	if (xSemaphoreBut_2 == NULL){
		printf("falha em criar o semaforo2 \n");
	}
	if (xSemaphoreBut_3 == NULL){
